...
import { Match, Type as EventType, Game, GameMode, Mod as LobbyMod, Team, TeamType, User, Score } from "../definitions/Match";
...
export class Flashlight extends DiscordClient {
    ...
    async fetchMultiplayer(lobby: string, options?: { 
        multipliers?: FlashlightOptions.MatchCosts.Mods, 
        mapIndex?: FlashlightOptions.MatchCosts.mapIndexing
    }): Promise<FlashlightOptions.MatchCosts.Return> {
        let eventIdCursor = 0;
        const modEnum = {
            'NM': 1, 'NF': 1, 'EZ': 1,
            'HD': 1 , 'HR': 1,'SD': 1, 
        ...
        if (options?.hasOwnProperty("multipliers")) {
            for (const mod in options.multipliers) {
                const currMod: LobbyMod = LobbyMod[mod as keyof typeof LobbyMod];
